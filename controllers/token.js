...
module.exports = {
    confirmation: function( req, res, next ) {
        Token.findOne({token: req.params.token}, ( err, token ) => {
            if(token == null) return res.status(400).send({type: 'not-verified', msg: 'No se encontró token con este usuario. Quizá haya expirado y debas soliticar un nuevo token.'});
